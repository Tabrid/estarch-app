...
const ExtraSection6 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (extraSectionData?.type6 === 'Category') {
                    const productsRes = await fetch(`${baseUrl}/api/products/products/category/products/home/${encodeURIComponent(extraSectionData?.name6)}`);
                    const productsData = await productsRes.json();
                    setProducts(productsData);

                    // Fetch category details
                    const categoryRes = await fetch(`${baseUrl}/api/categories/find/${extraSectionData?.id6}`);
                    const categoryData = await categoryRes.json();
                    setCategory(categoryData);
                    console.log("category;");
                    

                } else if (extraSectionData?.type6 === 'Subcategory') {
                    console.log("subcategory");
                    const subcategoryRes = await fetch(`${baseUrl}/api/products/products/subcategory/home/${encodeURIComponent(extraSectionData?.name6)}`);
                    ...
                    const subcategoryResponse = await fetch(`${baseUrl}/api/categories/subcategories/find/${extraSectionData?.id6}`);
        ...
        fetchData();
    }, [extraSection?.id6, extraSection?.name6, extraSection?.type6]);
    ...
    return (
        <SafeAreaView >
            <Text className='text-center mb-4 font-semibold text-[16px]'>{extraSection?.name6}</Text>
            <TouchableOpacity
                onPress={() => {
                    router.push(`/homeAllProduct/${extraSection?.type6}/${products[0]?.selectedType}/${extraSection?.name6}`); // Navigate to a special category     
                }}
            >
                <Image
