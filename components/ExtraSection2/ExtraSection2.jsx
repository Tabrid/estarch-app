...
const ExtraSection2 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (extraSectionData?.type2 === 'Category') {
                    const productsRes = await fetch(`${baseUrl}/api/products/products/category/products/home/${encodeURIComponent(extraSectionData?.name2)}`);
                    const productsData = await productsRes.json();
                    setProducts(productsData);
                    // Fetch category details
                    const categoryRes = await fetch(`${baseUrl}/api/categories/find/${extraSectionData?.id2}`);
                    const categoryData = await categoryRes.json();
                    setCategory(categoryData);
                } else if (extraSectionData?.type2 === 'Subcategory') {
                    const subcategoryRes = await fetch(`${baseUrl}/api/products/products/subcategory/home/${encodeURIComponent(extraSectionData?.name2)}`);
                    ...
                    const subcategoryResponse = await fetch(`${baseUrl}/api/categories/subcategories/find/${extraSectionData?.id2}`);
        ...
        fetchData();
    }, [extraSection?.id2, extraSection?.name2, extraSection?.type2]);
    ...
    //             fetch(`${baseUrl}/api/products/products/category/products/home/${encodeURIComponent(extraSection?.name2)}`)
    //                 .then((res) => res.json())
    //                 .then((data) => {
    //                     setProducts(data);
    //                     setLoading(false);
    ...
    return (
        <View className='mx-4'>
            ...
            <TouchableOpacity
                onPress={() => {
                    router.push(`/homeAllProduct/${extraSection?.type2}/${products[0]?.selectedType}/${extraSection?.name2}`); // Navigate to a special category     
                }}
            >
                <Image
