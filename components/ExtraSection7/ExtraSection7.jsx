...
const ExtraSection7 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (extraSectionData?.type7 === 'Category') {
                    const productsRes = await fetch(`${baseUrl}/api/products/products/category/products/home/${encodeURIComponent(extraSectionData?.name7)}`);
                    const productsData = await productsRes.json();
                    setProducts(productsData);
                    // Fetch category details
                    const categoryRes = await fetch(`${baseUrl}/api/categories/find/${extraSectionData?.id7}`);
                    const categoryData = await categoryRes.json();
                    setCategory(categoryData);
                } else if (extraSectionData?.type7 === 'Subcategory') {
                    const subcategoryRes = await fetch(`${baseUrl}/api/products/products/subcategory/home/${encodeURIComponent(extraSectionData?.name7)}`);
                    ...
                    const subcategoryResponse = await fetch(`${baseUrl}/api/categories/subcategories/find/${extraSectionData?.id7}`);
        ...
        fetchData();
    }, [extraSection?.id7, extraSection?.name7, extraSection?.type7]);
    ...
    //     React.useCallback(() => {
    //         setLoading(true);
    //         if (extraSection?.type7 === 'Category') {
    //             fetch(`${baseUrl}/api/products/products/category/products/home/${encodeURIComponent(extraSection?.name7)}`)
    //                 .then((res) => res.json())
    //                 .then((data) => {
    //                     setProducts(data);
    //                     setLoading(false);
    ...
    //         } else if (extraSection?.type7 === 'Subcategory') {
    //             fetch(`${baseUrl}/api/products/products/subcategory/home/${encodeURIComponent(extraSection?.name7)}`)
    //                 .then((res) => res.json())
    //                 .then((data) => {
    ...
    return (
        <View className='mx-4 mb-16'>
            <Text className='text-center my-4 font-semibold text-[16px]'>{extraSection?.name7}</Text>
            <TouchableOpacity
                onPress={() => {
                    router.push(`/homeAllProduct/${extraSection?.type7}/${products[0]?.selectedType}/${extraSection?.name7}`); // Navigate to a special category     
                }}
            >
                <Image
